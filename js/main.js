...
// =============================== THEME (LIGHT / DARK) ===============================
// 起動時は localStorage の保存値 → 無ければ OS の prefers-color-scheme を尊重 → デフォルト dark。
// 切替はヘッダの ☀️/🌙 ボタンから toggleTheme() で。テーマは <html data-theme="..."> に反映。
const _THEME_KEY = 'if-dashboard:theme:v1';

function _resolveInitialTheme(){
  // localStorage が使える環境なら保存値を読む
  try {
    const saved = localStorage.getItem(_THEME_KEY);
    if(saved === 'light' || saved === 'dark') return saved;
  } catch(e){ /* 取得失敗時は OS 設定にフォールバック */ }
  // OS のダークモード設定
...
function applyTheme(theme){
  // 反映：data-theme 属性で CSS 変数群を一気に差し替え
  document.documentElement.dataset.theme = theme;
  // 永続化（失敗しても致命的ではない）
  try { localStorage.setItem(_THEME_KEY, theme); } catch(e){}
  // ボタンアイコン更新（現在 dark なら ☀️ = 押すと light に切替）
  const icon = document.getElementById('themeToggleIcon');
  if(icon) icon.textContent = theme === 'light' ? '🌙' : '☀️';
  // JS 側で色を保持している描画物（Chart.js / Leaflet タイル / Globe）を最新の CSS 変数で再適用
  if(typeof applyMapTheme === 'function') applyMapTheme();
  if(typeof applyGlobeTheme === 'function') applyGlobeTheme();
  // チャート・バー・地図ルートなど全描画を再実行（cssVar() を読み直す）
...
// 初期化：DOM 完成後に呼ぶ。CSS の :root は dark がデフォルトなので、light のときだけ属性を立てる。
// （prefers-color-scheme が light で localStorage 未保存なら light、それ以外 dark）
(function _bootstrapTheme(){
  const t = _resolveInitialTheme();
  document.documentElement.dataset.theme = t;
  // アイコンは DOMContentLoaded 後に更新（要素がまだ無い可能性があるため）
