...
// 控えめなトーストで「✓ Saved」を短時間表示。
// 頻度の高い操作（Import 一括等）で連続発火しても showToast 側で上書きされる。
let _lastSavedToastAt = 0;
function _onAutoSave(){
  _setSaveStatus('ok');
  const now = Date.now();
  // 同じ操作内で連続発火する場合があるので、100ms 以内は間引く
  if(now - _lastSavedToastAt < 100) return;
  _lastSavedToastAt = now;
  showToast('✓ Saved');
}

// 保存失敗時：アイコンを警告状態に切り替える。ユーザーが状況を理解できる文言に。
// （引数 _err は将来のエラー種別ごとの分岐用に予約。現状は state のみで判定。）
...
function showRestoreModal(summary){
  ...
  const dateWrap = document.getElementById('restoreDateWrap');
  if(summary.latestDate){
    document.getElementById('restoreDate').textContent = summary.latestDate;
...
// 次回新セッション時にまた同じモーダルが出る（PLAN2.md の B-2 仕様通り）。
