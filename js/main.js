...
// 上部の通知バナーで「✓ Auto-saved」を 30 秒表示（連続発火は間引き）。
...
// =============================== NOTIFY BANNER ===============================
// 上部スティッキーバナー。緑（成功）は 30 秒で自動消滅、赤（エラー）は ✕ で閉じるまで常設。
...
function showNotifyBanner(variant, message, opts){
  ...
  // success は 30 秒で自動消滅、error は ✕ で閉じるまで常設
  const persistent = opts && opts.persistent;
  if(!persistent){
    _notifyTimer = setTimeout(() => closeNotifyBanner(), 30000);
  }
}
