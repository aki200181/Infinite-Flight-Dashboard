...
//     を書くと dep=RJTT かつ arr=RJOO の行に絞る
...
function filterFlights(){
  ...
  const includes = []; // string (lowercase)
  ...
  for(const t of tokens){
    ...
    if(m){
      routes.push({ dep: m[1].toUpperCase(), arr: m[2].toUpperCase() });
    } else {
  ...
  const d = getFiltered().filter(f => {
    ...
    for(const inc of includes){ if(!hay.includes(inc)) return false; }
